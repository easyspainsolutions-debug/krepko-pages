...
/* Лента отзывов переехала в reviews-carousel.js (2026-08-07): тот же блок
   теперь стоит и на страницах услуг, а тянуть туда весь этот файл незачем. */
...
(function () {
  ...
  function apply(p) {
    /* первые 16% пробега колода просто лежит под фото — её успевают
       разглядеть, и только потом начинается наезд */
    var pa = Math.max(0, (p - 0.16) / 0.84);
    cards.forEach(function (c, i) {
      var pi = Math.max(0, Math.min(1, pa * (N + 0.55) - i * 1.0));
      var eb = easeOutBack(pi);
      var ec = easeOutCubic(pi);
      /* Карточки видны стопкой ПОД фото ещё до начала наезда: каждая
         ждёт своей очереди чуть ниже предыдущей, выглядывая корешком
         (как у референса). Отсчёт от центра экрана, где стоит фото. */
      /* колода ждёт у нижнего края экрана: наружу выглядывают только
         тонкие корешки, срез нижней гранью в глаза не бросается */
      var wait = 64 + i * 2.5;                       /* vh, позиция ожидания */
      var park = i * 2.4;                            /* vh, место в стопке */
      var y = park + (1 - eb) * (wait - park);
      c.style.setProperty('--y', y.toFixed(2));
      /* стопка ложится неаккуратно: у каждого слоя свой угол и снос.
         Карточки уже фото, поэтому углы остаются внутри кадра. */
