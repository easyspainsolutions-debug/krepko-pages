...
/* Round 43: лента отзывов — стрелки и их гашение на краях. */
(function(){
  ...
  /* Листаем по индексу, а не относительным сдвигом: scrollBy накапливал
     дробные пиксели и отзыв вставал криво, наполовину показывая соседний. */
  ...
  /* ручная прокрутка/свайп — пересчитываем индекс по ближайшей карточке */
  var t = null;
  track.addEventListener('scroll', function(){
    if (t) clearTimeout(t);
    t = setTimeout(function(){
      var w = track.clientWidth;
      if (w) { idx = Math.round(track.scrollLeft / w); sync(); }
    }, 90);
  }, {passive: true});
